...
class ChatMessages(DeclarativeBase.Base):
    ...
    def __init__(self, object_chat_session: ChatSessionUsers.ChatSessionUsers, message):
    ...
    @classmethod
    def _send_message(cls, channel_id, user_id, msg):
        db = DatabaseManager.DatabaseManager.get_session()
        try:
            session = db.query(ChatSessionUsers.ChatSessionUsers).filter(
                ChatSessionUsers.ChatSessionUsers.chat_session_id == channel_id,
    ...
    @classmethod
    async def send_message(cls, channel_id, user_id, msg):
        return await asyncio.get_event_loop().run_in_executor(None, partial(cls._send_message, channel_id, user_id, msg))
